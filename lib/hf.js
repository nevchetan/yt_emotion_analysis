 ...
 *
 * Provides fast emotion classification using either:
 * 1. Local Python FastAPI server (10-50ms) - Primary choice
 * 2. Hugging Face Inference API (fallback, ~10+ seconds)
 *
...
export async function analyzeEmotion(text) {
  ...
  // Strategy 1: Local FastAPI server (fastest)
  try {
    ...
    // Local server unavailable, continue to fallback
  }
 ...
 * Uses batch endpoint on local server, falls back to sequential HF API calls
 * @param {string[]} texts - Array of texts to analyze
 * @param {number} concurrency - Number of parallel requests (default 2)
 * @returns {Promise<Array<{label: string, score: number}>>} Array of emotions
 */
export async function analyzeEmotionsBatch(texts, concurrency = 2) {
  ...
  // Try local server batch endpoint first (processes all at once)
  try {
    const controller = new AbortController();
